...
# Overwrite config.js with a production build pointing at the backend service.
cat > "$BUILD/frontend/site/js/config.js" <<EOF
/* Production config generated by deploy.sh — API_BASE points at Cloud Run. */
window.SPS_CONFIG = {
...
EOF
ok "Frontend API_BASE set to $BACKEND_URL"
